...
export function DevAccess() {
  ...
  useEffect(() => { fetch("/api/dev-session").then((r) => r.json()).then((v) => setEnabled(Boolean(v.enabled))).catch(() => undefined); }, []);
  ...
  return (
    <div className="mt-6 border-t border-white/10 pt-6">
      <button
        ...
        type="button"
      >
        <Code2 size={16} /> {busy ? "Opening local workspace…" : "Enter local developer workspace"}
      </button>
