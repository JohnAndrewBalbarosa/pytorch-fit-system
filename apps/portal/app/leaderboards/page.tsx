...
import { rankForPoints, type LeaderboardPayload, type LeaderboardView } from "@pytorch-fit/domain-protocol/leaderboards";
  ...
  return <AppShell>
    <div className="space-y-5">
      <section className="rounded-2xl border border-white/10 bg-[#141416] p-5 lg:p-7" data-tour="leaderboards-heading">
        ...
        <div aria-label="Evidence verification view" className="mt-4 flex flex-wrap gap-2" role="tablist">{(["both","verified","pending"] as const).map((item) => <button aria-selected={view===item} className={`rounded-full border px-4 py-2 text-sm font-semibold ${view===item ? "border-success bg-success/10 text-success" : "border-border text-muted"}`} key={item} onClick={() => { setView(item); setPage(1); }} role="tab" type="button">{item === "both" ? "Verified + pending" : item[0].toUpperCase()+item.slice(1)}</button>)}</div>
